...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.tableWidget.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers) #只读
    ...
    def init_data(self):
        ...
        for dt in csdnCg.getData():
            for i in dt:
                ...
                label = QtWidgets.QLabel("<a href='{0}'>{1}</a>".format(i[0],i[1]))
                ...
                self.number[1]+=int(i[4])
        self.label.setText("总浏览量：%d  总评论量：%d" % (self.number[0] , self.number[1]))
    ...
    def define_old(self):
        ...
        self.label.setText("总浏览量：%d  总评论量：%d " % (self.number[0], self.number[1]))
        self.label.setText(self.label.text()+"总浏览量增加：%d  总评论增加：%d " % (total[0],total[1]))
